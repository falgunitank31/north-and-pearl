...
- August 8 DataForSEO setup status: official local MCP workflow is registered in Codex via `~/.codex/bin/dataforseo-mcp.sh`, field config is installed, and cost controls are configured at threshold mode with `$0.10` approval threshold and `$2.00` daily limit. Current 7-day DataForSEO ledger shows `$0.00` spend and 0 calls. Live connection validation is blocked until API credentials are added securely.
