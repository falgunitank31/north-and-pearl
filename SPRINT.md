# North & Pearl Sprint 1

Sprint dates: July 27, 2026 - August 2, 2026

## Sprint Goal

Make the live storefront more premium, shoppable, and marketing-ready before scaling traffic.

## High-Impact Objectives

1. Fix P1 storefront presentation problems that reduce trust or product discovery.
2. Improve product/catalog visual confidence for active products.
3. Strengthen organic discovery paths without adding thin content.
4. Verify Merchant Center and Search Console readiness.
5. Establish the permanent measurement and documentation loop.

## Agent Assignments

- Lead Orchestrator: prioritize, coordinate, QA, document, and prevent scope drift.
- Tesla: implement theme fixes for navigation, collections, PDP/card behavior, performance, and accessibility.
- Kuhn: validate premium jewelry design quality, spacing, visual hierarchy, and product image presentation.
- Gauss: audit active products, image readiness, merchandising, collections, prices, variants, and availability.
- Faraday: validate SEO/AEO/GEO, internal linking, Search Console, metadata, and commercial content paths.
- Rawls: confirm GA4/Search Console measurement readiness and define baseline reporting.
- Lovelace: verify shipping/returns/customer-support promises are operationally safe.
- Curie: verify trust claims, supplier/material evidence boundaries, and customer objection gaps.

## Current P1 Execution
...
- [x] Convert the order-growth operating model from continuous work into a daily goal cadence with one daily execution block, measurable daily targets, and a stop rule unless P0/P1 issues appear.
...
- [x] Completed accessible August 11 Batch 1 screening and first approved Shopify creation: 3/17 candidates image/IP screened, 2 GREEN products published with exact Alibaba source mapping, 1 YELLOW product held, and 14 candidates held until Alibaba visual-screening access clears.
...
- [x] Added collection featured images for all 13 populated commercial collections using representative live product imagery.
- [x] Updated homepage category and occasion cards to prefer collection featured images before product fallbacks.
...
- [x] Verified the Google API blocker remains local-credential related: Codex SEO has no API key, OAuth token, service account, GA4 property ID, or Search Console property configured at `/Users/yagneshtank/.config/codex-seo/google-api.json`.
...
- [x] Strengthened internal links from indexed collection pages into Gifts Under $50 and the weekly gift guide; Theme Check passed and the update was pushed live.
- [x] Added a homepage internal link to the weekly gift guide and validated the key discovery surfaces plus representative cart add.
...
- [x] Ran the August 4 same-thread operating pass: Search Console, sitemap status, URL Inspection, GA4 organic, safe order read, product SEO, Merchant readiness, source-image queue, menu audit, Theme Check, full live storefront QA, and command center evidence.
...
- [x] Ran August 6 monitor-only commerce QA while GSC indexing stabilizes: product SEO, Merchant readiness, product media, priority collection quality, full live PDP QA, and cart add test.
- [x] Ran Faraday/DataForSEO blog-content analysis on August 8: 17 blog sitemap URLs inspected, 15 articles scored, 49 keyword targets checked, and the priority content queue now focuses on duplicate intent cleanup, product/collection links, buyer-decision blocks, FAQs, and Article schema validation.
- [x] Published three DataForSEO-selected commercial Gift Guide articles: Charm Bracelet Guide, Initial Necklace Guide, and Birthstone Necklace Guide. All three live URLs return 200 and contain three direct product links plus relevant collection links.
- [x] Fixed the two GSC Merchant listings warnings requested by the owner: added conservative Product schema `deliveryTime` and corrected `returnPolicyCategory`; Theme Check passed and live PDP JSON-LD validation passed.
- [x] Established tracked order goals for the next two operating windows: 7-Day First-Order Push through August 15, 2026 and 30-Day Order Traction through September 7, 2026; added them to the Agent Command Center and project metrics.
- [x] Tesla/Rawls connected aggregate GA4 goal-window funnel pulls into the Agent Command Center: sessions, product views, add-to-carts, checkout starts, purchases, and purchase revenue now come from `scripts/ga4-order-goal-progress.py`.
- [x] Pareto/Rawls/Faraday recorded the first order-goal checkpoint for August 8: 1 session, 0 product views, 0 add-to-carts, 0 checkout starts, 0 purchases.
- [x] Pareto/Kuhn/Tesla added a homepage "Shop a piece now" direct-product rail from the featured collection to address the measured session-to-product-view gap.
- [x] Pareto/Faraday/Kuhn/Tesla added direct product cards to the shared Gift Guide link section so blog/article landing pages can move visitors into PDPs faster.
- [x] Pareto/Kuhn/Tesla fixed the homepage "Make the moment easy to shop" merchandising imbalance: 8 occasion cards now resolve to a balanced 4-column desktop grid instead of an awkward 5 + 3 layout; Theme Check passed and the live homepage CSS validates the new rule.
- [x] Pareto/Faraday/Kuhn/Tesla strengthened highest-intent collection landing pages with compact "Start with these pieces" direct-product strips; sampled live collections return 200 and show 3 PDP jump links each.
...
- [x] Pareto/Rawls/Faraday daily goal tracking for August 8: refreshed verified sessions, product clicks, product views, add-to-carts, checkout starts, orders, Search Console clicks, sitemap status, Merchant readiness, sampled storefront URLs, and Theme Check against the daily, 7-day, and 30-day targets.
...
- [x] Kuhn/Gauss/Pareto/Tesla diversified the first homepage product-entry rail so the "Shop a piece now" section starts with a necklace, bracelet, ring, and earrings path instead of a repetitive heart-necklace sequence.
- [x] Tesla/Kuhn/Pareto added homepage hero "Popular picks" PDP links to target the current 0 product-click funnel gap; remote theme files confirm deployment, while immediate public storefront HTML checks were still cached.
