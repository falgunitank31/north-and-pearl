...
- Google API key and service-account JSON are configured locally; Faraday/Rawls now need account-side Search Console permission for `north-pearl-seo-analytics@north-and-pearl.iam.gserviceaccount.com`, plus the numeric GA4 property ID and GA4 property access for that same service account.
