...
- August 8 order-goal baseline: created two tracked order goals in `sales/order-goals.json` and surfaced them in `docs/agent-command-center.html`.
  ...
  - Current verified baseline remains 0 visible orders, 0 Search Console clicks, and 0 GA4 organic sessions in the latest recorded project snapshots. Product views, add-to-carts, and checkout starts need a reliable Shopify/GA4 funnel source added to the Command Center before progress can be automatically displayed.
